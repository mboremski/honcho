...
[EXPLICIT] DEFINITION: Facts about {peer_id} that can be derived directly from their messages.
   ...
   - Use absolute dates/times when possible (e.g. "June 26, 2025" not "yesterday"), but ONLY when the message actually provides them

CRITICAL — DO NOT FABRICATE:
- Extract observations ONLY from the actual content inside the <messages> block below.
- NEVER output a fact that is not supported by those messages. Do not invent ages, birth dates, locations, pets, jobs, schooling, or any other detail.
- The EXAMPLES below illustrate FORMAT ONLY. Their fictional person, places, dates and values must NEVER appear in your output.
- If the messages contain no extractable facts about {peer_id}, return no observations rather than inventing any.
...
- Extract ALL real observations from {peer_id}'s messages, using others as context.
- Contextualize each observation sufficiently (e.g. "Ann is nervous about the job interview at the pharmacy" not just "Ann is nervous")

EXAMPLES (fictional — these show the INPUT→OUTPUT format ONLY; never copy their content into your output):
- Message "I just had my 25th birthday yesterday" → "EXAMPLE_PERSON is 25 years old", and a birthday date ONLY if it can actually be computed from the message timestamp
- Message "I took my dog for a walk near home in <some city>" → "EXAMPLE_PERSON has a dog", "EXAMPLE_PERSON lives in <that city>"
- Message "I went to college" + general knowledge → "EXAMPLE_PERSON completed high school or equivalent"
